...
class GaussFit(object):
	...
	def do_fit(self, guess=[]):
		...
		fit , cov = scipy.optimize.curve_fit(
			...
			self.data_x, # data x points
			self.data_y, # data y points
			p0=[self.GUESS[0],self.GUESS[1],self.GUESS[2]], # initial guess
			sigma=self.data_err) # error bars for the points
		self.fit = fit # set the class fit variable
		self.covariance = cov # set the class covariance variable
	...
	def chi2_other(self, fit):
		...
		for point in self.data:
			...
			if( self.OPT_RESTRICT_CHI2 and math.fabs(point[0]-self.fit[1])/self.fit[2] <= 5 ):
	...
	def red_chi2_other(self,fit):
		return math.sqrt( self.chi2_other(fit) / len(self.data) )
	...
	def chi2_fit_unc(self):
		...
		for sign in [-1,1]:
			...
			while (chi2 - chi2_min) < 1:
				...
				new_fit , cov = scipy.optimize.curve_fit(
					lambda x,mu,sigma: Gaussian(x,self.fit[0]+d,mu,sigma),
					self.data_x,
					self.data_y,
					p0=[self.fit[1],self.fit[2]],
					sigma=self.data_err)
		...
		for sign in [-1,1]:
			...
			while (chi2 - chi2_min) < 1:
				...
				# calculate a new best fit:
				new_fit , cov = scipy.optimize.curve_fit(
					lambda x,A,sigma: Gaussian(x,A,self.fit[1]+d,sigma),
					self.data_x,
					self.data_y,
					p0=[self.fit[0],self.fit[2]],
					sigma=self.data_err)
		...
		for sign in [-1,1]:
			...
			while (chi2 - chi2_min) < 1:
				...
				# calculate a new best fit:
				new_fit , cov = scipy.optimize.curve_fit(
					lambda x,A,mu: Gaussian(x,A,mu,self.fit[2]+d),
					self.data_x,
					self.data_y,
					p0=[self.fit[0],self.fit[1]],
					sigma=self.data_err)
