...
import Analysis.Hohlraum as Hohlraum
import math
...
class TestHohlraum(TestCase):
    ...
    def test_get_data_raw(self):
        ...
        for i in range(len(self.data)):
            self.assertTrue(all(self.data[i][j] == hData[i][j] for j in range(len(hData[i]))), msg="Failed raw data comparison.")
    ...
    def test_get_unc(self):
        ...
        self.assertAlmostEqual(unc[0][0], 4663.95, delta=1)
        self.assertAlmostEqual(unc[0][1], 3965.73, delta=1)
